...
> **Nota:** o enunciado sugere SQL Server como banco de dados. Optei por MySQL porque já tinha o MySQL Workbench configurado localmente e o SQL Server LocalDB não estava disponível na minha máquina no momento do desenvolvimento. A arquitetura, o Code First, as Migrations e toda a lógica de persistência com EF Core são idênticas ao que seria feito com SQL Server — troca-se apenas o provider (`Pomelo.EntityFrameworkCore.MySql` em vez de `Microsoft.EntityFrameworkCore.SqlServer`) e a connection string em `appsettings.json`.
